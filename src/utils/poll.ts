 ...
 * @param {string[]} options Options from the poll creation message
 * @param {string[]} pollOptionHashes hash from `decryptPollMessageRaw()`
 ...
 * @param {Uint8Array} encPayload Encryption payload/contents want to decrypt, you can get it from `pollUpdateMessage.vote.encPayload`
 * @param {Uint8Array} encIv Encryption iv (used to decrypt the payload), you can get it from `pollUpdateMessage.vote.encIv`
 * @param {Uint8Array} additionalData poll Additional data to decrypt poll message
 * @param {Uint8Array} decryptionKey Generated decryption key to decrypt the poll message
 ...
 * @param {Uint8Array} decryptedMessage the message from `decrpytPollMessageInternal()`
 * @return {string}
 */
 ...
 * @param {Uint8Array} encKey Encryption key (used to decrypt the payload), you need to store/save the encKey. If you want get the encKey, you could get it from `Message.messageContextInfo.messageSecret`, only available on poll creation message.
 * @param {Uint8Array} encPayload Encryption payload/contents want to decrypt, you can get it from `pollUpdateMessage.vote.encPayload`
 * @param {Uint8Array} encIv Encryption iv (used to decrypt the payload), you can get it from `pollUpdateMessage.vote.encIv`
 * @param {string} pollMsgSender sender The sender's jid of poll message, you can use `pollUpdateMessage.pollCreationMessageKey.participant` (Note: you need to normalize the jid first)
 * @param {string} pollMsgId The ID of poll message, you can use `pollUpdateMessage.pollMessageCreationKey.id`
 * @param {string} voteMsgSender The poll voter's jid, you can use `Message.key.remoteJid`, `Message.key.participant`, or `Message.participant`. (Note: you need to normalize the jid first)
